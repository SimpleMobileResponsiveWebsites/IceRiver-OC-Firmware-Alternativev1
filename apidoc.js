The document describes the API (Application Programming Interface) for a modified IceRiver ASIC firmware, providing a way to interact with the miner's functions and data programmatically. 
  
Here's a summary of the API, drawing from the provided sources:●General API Information:
    
○Requests are made over HTTPS on port 443.

○Requests use HTTP GET or HTTP POST (when parameters are needed)1.

○Authentication is done via API token using a header of the form Authorization: Bearer <token>.
